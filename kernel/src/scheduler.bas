...
sub ThreadQueue.EnqueueRange(tFirst as Thread ptr,tLast as Thread ptr)
    if (this.FirstThread<>0) then
...
sub ThreadScheduler.IncreasePriority(i as unsigned integer)
    if (i>0) then
        if (PriorityQueue(i).FirstThread <> 0) then
            ...
            PriorityQUeue(i-1).EnqueueRange(PriorityQueue(i).FirstThread,PriorityQueue(i).LastThread)
