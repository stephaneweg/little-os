...
sub MAIN(p as any ptr) 
	...
	MainWin = WindowCreate(285,225,@"Calculator")
	DisplayTxt = TextBoxCreate(MainWin,10,10,265,30)
    
    btnCos = ButtonCreate(MainWin,10,45,40,30,@"COS",@btnFuncClick,0)
    btnSin = ButtonCreate(MainWin,55,45,40,30,@"SIN",@btnFuncClick,0)
    
    
    btnNum7	= ButtonCreate(MainWin,10,80,40,30,@"7",@BTNNumClick,0)
    btnNum8	= ButtonCreate(MainWin,55,80,40,30,@"8",@BTNNumClick,0)    
    btnNum9	= ButtonCreate(MainWin,100,80,40,30,@"9",@BTNNumClick,0)
    
    btnNum4	= ButtonCreate(MainWin,10,115,40,30,@"4",@BTNNumClick,0)
    btnNum5	= ButtonCreate(MainWin,55,115,40,30,@"5",@BTNNumClick,0)
    btnNum6	= ButtonCreate(MainWin,100,115,40,30,@"6",@BTNNumClick,0)
    
    btnNum1	= ButtonCreate(MainWin,10,150,40,30,@"1",@BTNNumClick,0)
    btnNum2	= ButtonCreate(MainWin,55,150,40,30,@"2",@BTNNumClick,0)
    btnNum3	= ButtonCreate(MainWin,100,150,40,30,@"3",@BTNNumClick,0)
    
    btnNum0 = ButtonCreate(MainWin,10,185,40,30,@"0",@BTNNumClick,0)
    ButtonCreate(MainWin,55,185,40,30,@"+/-",@BTNPlusMinusClick,0)
    btnDot	= ButtonCreate(MainWin,100,185,40,30,@".",@BTNNumClick,0)
    
    ButtonCreate(MainWin,145,80,40,30,@"+",@BTNPlusClick,0)
    ButtonCreate(MainWin,145,115,40,30,@"-",@BTNMinusClick,0)
    ButtonCreate(MainWin,145,150,40,30,@"*",@BTNMultiplyClick,0)
    ButtonCreate(MainWin,145,185,40,30,@"/",@BTNDivideClick,0)
    
    ButtonCreate(MainWin,190,80,85,30,@"C",@BTNClearClick,0)
    btnSQRT = ButtonCreate(MainWin,190,115,40,30,@"SQRT",@btnFuncClick,0)
    ButtonCreate(MainWin,190,150,40,30,@"PI",@BTNPIClick,0)
    btnSQR  = ButtonCreate(MainWin,235,115,40,30,@"x^2",@btnFuncClick,0)
    
    
    ButtonCreate(MainWin,235,150,40,30,@"%",@BTNPERCENTClick,0)
    ButtonCreate(MainWin,190,185,85,30,@"=",@BTNEqualsClick,0)
    WaitForEvent()
	do:loop
end sub
...
sub DoOperation()
	TextBoxGetText(DisplayTxt,txtX)
    valuex = Atof(txtX)
...
sub BTNEqualsClick(btn as unsigned integer,parm as unsigned integer)
    DoOperation()
    ftoa(Value,txtX)
    TextBoxSetText(displayTxt,txtX)
...
sub BTNPIClick(btn as unsigned integer,parm as unsigned integer)
    TextBoxSetText(displayTxt,@"3.14159265358979323846")
	EndCallBack()
end sub
...
sub BTNNumClick(btn as unsigned integer,parm as unsigned integer)
    if (IsOPComplete = 1) then TextBoxSetText(displayTxt,@""):IsOPComplete=0
	
	if (btn=btnnum0) then 
		TextBoxAppendChar(displayTxt,asc("0"))
	elseif (btn=btnnum1) then 
		TextBoxAppendChar(displayTxt,asc("1"))
	elseif (btn=btnnum2) then 
		TextBoxAppendChar(displayTxt,asc("2"))
	elseif (btn=btnnum3) then 
		TextBoxAppendChar(displayTxt,asc("3"))
	elseif (btn=btnnum4) then 
		TextBoxAppendChar(displayTxt,asc("4"))
	elseif (btn=btnnum5) then 
		TextBoxAppendChar(displayTxt,asc("5"))
	elseif (btn=btnnum6) then 
		TextBoxAppendChar(displayTxt,asc("6"))
	elseif (btn=btnnum7) then 
		TextBoxAppendChar(displayTxt,asc("7"))
	elseif (btn=btnnum8) then 
		TextBoxAppendChar(displayTxt,asc("8"))
	elseif (btn=btnnum9) then 
		TextBoxAppendChar(displayTxt,asc("9"))
	elseif (btn=btnDot) then
		if (hasDot=0) then
			TextBoxAppendChar(displayTxt,asc("."))
			hasdot=1
		end if
	end if
	EndCallBack()
