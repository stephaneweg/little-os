...
sub IRQ_SET_THREAD_HANDLER(intno as unsigned integer,th as any ptr,e as unsigned integer)
    if (IRQ_THREAD_HANDLERS(intno).Owner = 0) then
...
sub IRQ_THREAD_HANDLER.Enqueue()
    this.Counter += 1
...
function IRQ_THREAD_HANDLER.Dequeue() as unsigned integer
