...
sub MAIN(p as any ptr) 
	...
	MainWin = WindowCreate(490,scrollViewHeight+5,@"Applications")
    ...
    for i = 0 to cpt-1
            if (strendswith(@(DirEntries[i].FileName(0)),@".APP")) then
                ...
                row = num \ 3
                num+=1
                var btn = ButtonCreate(buttonContainer, col * (btnHeight+btnSpace)+btnSpace,row*(btnHeight+btnSpace)+btnSpace,btnHeight,btnHeight,@(DirEntries[i].FileName(0)),@AppButtonClick,i)
                strcpy(@executablePath(0),strcat(@"SYS:/APPS/",@(DirEntries[i].FileName(0))))
                strcpy(@executablePath(0),strcat(@executablePath(0),@"/app.bmp"))
                ButtonSetIcon(btn,@executablePath(0),1)
            
            end if
    next
    
    
    btnScrollUp     = ButtonCreate(drawable,460,0,30,30,@txtArrow(0),@btnScrollClick,1)
    btnScrollDown   = ButtonCreate(drawable,460,scrollViewHeight-30,30,30,@txtArrow(2),@btnScrollClick,2)
